...
// ──────────────────────────────────────────────
// Constants — totalSections = jumlah chapter/bab
// ──────────────────────────────────────────────
...
export default function HadithReader() {
  ...
  const [hadithIndex, setHadithIndex] = useState(0); // index dalam section
  ...
  const fetchSection = useCallback(async (bookId: string, sec: number) => {
    ...
    try {
      ...
      if (items.length === 0) throw new Error("Tidak ada hadits di section ini");

      // Simpan metadata section
      const meta: Record<string, SectionMeta> = data.metadata?.section_detail ?? {};
  ...
  // Navigasi hadith dalam section
  const prevHadith = () => {
    if (hadithIndex > 0) { setHadithIndex(hadithIndex - 1); return; }
    if (section > 1) { setSection(section - 1); /* index akan reset ke 0 */ }
  };
    ...
    <div className="max-w-2xl mx-auto px-4 pb-8 space-y-5">
          ...
          <div role="listbox" className="absolute top-full left-0 right-0 mt-2 z-50 rounded-2xl border border-secondary/15 dark:border-secondary/10 bg-white dark:bg-surface shadow-xl overflow-hidden">
            {BOOKS.map((book) => (
              <button
                key={book.id}
                role="option"
                aria-selected={selectedBook.id === book.id}
                onClick={() => { setSelectedBook(book); setSection(1); setHadithIndex(0); setBookDropdownOpen(false); }}
      ...
      <div className="flex items-center gap-2">
        <button
          id="hadith-prev-btn"
          onClick={prevHadith}
          disabled={(section <= 1 && hadithIndex === 0) || loading}
        ...
        <div className="flex-1 text-center">
          <p className="text-[11px] text-gray-400 font-medium">Bab {section} / {selectedBook.totalSections}</p>
          {sectionInfo && (
        ...
        <button
          id="hadith-next-btn"
          onClick={nextHadith}
          disabled={(section >= selectedBook.totalSections && hadithIndex >= hadiths.length - 1) || loading}
          ...
          {currentHadith && !loading && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-gray-800 dark:text-foreground leading-relaxed text-[15px]">
                  {cleanText(currentHadith.text)}
                </p>
              </div>
                  ...
                  {currentHadith.grades.map((g, i) => {
                    ...
                    return (
                      <span key={i} className="text-[10px] font-bold px-2.5 py-1 rounded-full border"
                        style={{
                          color: isShahih ? "#1A4D2E" : "#C9A84C",
                          borderColor: isShahih ? "#1A4D2E30" : "#C9A84C30",
                          backgroundColor: isShahih ? "#1A4D2E0D" : "#C9A84C0D",
                        }}>
                        {g.name}: {g.grade}
                      </span>
                    );
      ...
      <div className="flex items-center gap-3">
        <form
          onSubmit={(e) => { e.preventDefault(); const f = e.currentTarget; const v = parseInt((f.elements.namedItem("secNum") as HTMLInputElement).value, 10); if (!isNaN(v) && v >= 1 && v <= selectedBook.totalSections) setSection(v); }}
          className="flex flex-1 items-center gap-2"
        >
          <div className="relative flex-1">
            <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
            <input
              id="hadith-section-input"
              name="secNum"
              type="number"
              min={1}
              max={selectedBook.totalSections}
              defaultValue={section}
              key={`${selectedBook.id}-${section}`}
          ...
          <button type="submit" className="px-4 py-2.5 rounded-xl text-xs font-bold text-white transition-all active:scale-95"
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: "Bab Pertama", action: () => setSection(1) },
          { label: "Bab Acak", action: goRandom },
          { label: "Bab Terakhir", action: () => setSection(selectedBook.totalSections) },
        ].map((item) => (
