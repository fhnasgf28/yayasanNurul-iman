      ...
      >
        <div className="flex w-full items-end justify-between gap-1 rounded-[28px] border border-white/35 bg-gradient-to-b from-white/18 to-white/8 px-1 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.45)]">
          {primaryItems.map((item) => {
            const Icon = item.icon;
            const active = item.isActive(pathname, category);
          ...
          <button
            ...
            className={cn(
              "relative flex min-w-0 flex-1 flex-col items-center gap-1 rounded-[22px] px-2 py-2.5 transition-all active:scale-95",
              (isMoreOpen || isMoreRoute) && "bg-white/45 shadow-[0_8px_20px_rgba(26,77,46,0.08)] backdrop-blur-md"
            )}
          >
            <Circle
              size={7}
              className={cn(
                "transition-colors",
                isMoreOpen || isMoreRoute ? "fill-[#C8963E] text-[#C8963E]" : "fill-transparent text-transparent"
              )}
            />
            <Menu size={20} className={isMoreOpen || isMoreRoute ? "text-[#C8963E]" : "text-gray-400"} />
            <span className={cn("text-[11px] font-semibold", isMoreOpen || isMoreRoute ? "text-[#C8963E]" : "text-gray-400")}>
              Lainnya
            </span>
          </button>
        </div>
