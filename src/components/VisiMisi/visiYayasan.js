...
const VisiYayasan = () => {
  ...
  const [newData, setNewData] = useState({visi: '',})
  ...
  const addData = async (newData) => {
    try {
      const database = getDatabase(firebaseApp);
      const rootReference = ref(database, 'visi');
  ...
  const handleSearch = () => {
    ...
    for (const item of dataList) {
      if (
        (item.visi && item.visi.toLowerCase().includes(searchTerm.toLowerCase()))
      ) {
        results.push(item);
      }
    }
  ...
  const editDataItem = async (field, value, newData) => {
    try {
      const database = getDatabase(firebaseApp);
      const dataRef = ref(database, 'visi');
...
const handleEditClick = () => {
  ...
  editDataItem('visi', editData.visi, {
    visi: editData.visi,
  });
...
const closeDeleteModal = () => {
  ...
  return (
    <div className="bg-gray-100 p-8">
      <h1 className="text-2xl font-semibold mb-4">Visi Yayasan</h1>
   ...
   <div className="overflow-x-auto">
    <table className="table-auto min-w-full divide-y divide-gray-200">
      <thead className="bg-blue-600">
        <tr>
          <th scope="col"
        className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">visi</th>
        </tr>
      </thead>
      <tbody>
        {searchResults.map((item, index) => (
          <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}>
            <td className="px-6 py-4">{item.visi}</td>
    ...
    <form
      ...
      >
        <input
          ...
          placeholder="visi"
          value={newData.visi}
          onChange={(e) => setNewData({ ...newData, visi: e.target.value })}
        />
            ...
            <div className="bg-white p-4 rounded shadow-md">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  onClick={() => {
                    deleteDataByCondition('visi', dataToDelete.visi);
                    closeDeleteModal();
                  }}
                >
                  Hapus
